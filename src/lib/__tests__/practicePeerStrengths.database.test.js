...
it('collects peer strengths atomically and ranks only consented verified evidence', async () => {
  ...
  try {
    ...
    await expect(confirm(['synthesis','communication','structuring','hypothesis_development'])).rejects.toThrow('invalid_strength_skills')
    await confirm(['synthesis','responsive'])
    ...
    expect(cards[0].recommendation.practice_history).toBeNull()
    await as(2); await db.query('SELECT practice_history_sharing($1,true)',[c])
    await as(0); expect((await get())[0].recommendation.practice_history).toEqual({sessions:1,partners:1})
    await as(2); await db.query('SELECT practice_history_sharing($1,false)',[c])
    await as(0); expect((await get())[0].recommendation.practice_history).toBeNull()
    expect(cards[0].recommendation.peer_strengths).toEqual([{skill:'responsive',partners:1},{skill:'synthesis',partners:1}])
    expect(cards[0].recommendation.peer_relevant_skills).toEqual(['synthesis'])
    expect(cards[0].recommendation.shared_teammate_traits).toEqual([])
    await db.exec(`RESET ROLE; INSERT INTO practice_sessions(id,community_id,participant_a_user_id,participant_b_user_id,status,interview_category) VALUES('30000000-0000-4000-8000-000000000003','${c}','${ids[0]}','${ids[1]}','verified','case'); INSERT INTO practice_peer_strengths VALUES('30000000-0000-4000-8000-000000000003','${ids[1]}','${ids[0]}','${c}',ARRAY['responsive']);`)
    await as(0); await db.query('SELECT practice_peer_strengths_sharing($1,true)',[c])
    expect((await get())[0].recommendation.shared_teammate_traits).toEqual(['responsive'])
    await db.query('SELECT practice_peer_strengths_sharing($1,false)',[c])
    expect((await get())[0].recommendation.shared_teammate_traits).toEqual([])
    for(const id of ids) expect(JSON.stringify(cards)).not.toContain(id)
    expect((await db.query('SELECT * FROM practice_peer_strengths')).rows.every(r => r.recipient_user_id===ids[0] || r.author_user_id===ids[0])).toBe(true)
    ...
    await as(0); expect((await get())[0].recommendation.peer_strengths).toEqual([{skill:'responsive',partners:1},{skill:'synthesis',partners:1}])
