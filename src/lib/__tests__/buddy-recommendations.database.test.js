...
it('recommends only opted-in approved buddies and records interest without matching or identity release',async()=>{
 ...
 for(const name of ['migration-buddy-program.sql','migration-buddy-choice.sql','migration-buddy-recommendations.sql','migration-buddy-recommendations.sql'])await db.exec(await readFile(new URL(`../../../scripts/${name}`,import.meta.url),'utf8'))
 ...
 await as(0);await db.query('select buddy_choice_access($1,$2,true)',[p,'private2@example.test']);await db.query('select buddy_choice_access($1,$2,true)',[p,'private3@example.test'])
