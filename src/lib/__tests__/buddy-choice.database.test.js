...
it('protects manual buddy selection, first-year self enrollment, privacy and capacity',async()=>{
 ...
 await as(0);await db.query('select buddy_choice_access($1,$2,true)',[p,'s2@example.test']);await db.query('select buddy_choice_access($1,$2,true)',[p,'s3@example.test']);expect((await state()).posts).toHaveLength(0)
