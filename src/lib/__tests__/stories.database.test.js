...
beforeAll(async () => {
  ...
  await db.exec(await readFile(new URL('../../../scripts/migration-story-garden-notebook.sql', import.meta.url), 'utf8'))
...
describe('Private notebook and author-only warmth totals', () => {
  it('isolates covers by caller and community and rejects stale saves', async () => {
    const cover = await call(A, 'story_notebook_save', [C, 'lilac', 'flower', 'Quiet writer', 0])
    expect(cover.pen_name).toBe('Quiet writer')
    expect((await call(B, 'story_notebook_get', [C])).pen_name).toBe('')
    await expect(call(A, 'story_notebook_get', [OTHER])).rejects.toThrow('STORY_ACCESS')
    await expect(call(A, 'story_notebook_get', [C], 'anon')).rejects.toThrow(/permission denied/)
    await expect(call(A, 'story_notebook_save', [C, 'cream', 'leaf', 'Stale', 0])).rejects.toThrow('STORY_CONFLICT')
    await expect(call(A, 'story_notebook_save', [C, 'bogus', 'leaf', '', 1])).rejects.toThrow('STORY_INPUT')
    await call(A, 'story_notebook_get', [C])
    await expect(db.query('SELECT * FROM story_notebooks')).rejects.toThrow(/permission denied/)
  })
  it('counts distinct eligible readers privately and respects removal and blocks', async () => {
    // Other tests intentionally rerun the base migration; apply the upgrade again.
    await owner('SELECT 1')
    const upgrade = await readFile(new URL('../../../scripts/migration-story-garden-notebook.sql', import.meta.url), 'utf8')
    await db.exec(upgrade); await db.exec(upgrade)
    const story = await save()
    await call(B, 'story_set_reaction', [story.id, 'warmth', true])
    await call(B, 'story_set_reaction', [story.id, 'relate', true])
    await call(D, 'story_set_reaction', [story.id, 'warmth', true])
    expect((await call(A, 'story_get', [story.id])).received_reader_count).toBe(2)
    expect((await call(B, 'story_get', [story.id])).received_reader_count).toBeUndefined()
    expect((await list(B)).items[0].received_reader_count).toBeUndefined()
    await owner('INSERT INTO blocks(blocker_id,blocked_user_id) VALUES($1,$2)', [A,D])
    expect((await call(A, 'story_get', [story.id])).received_reader_count).toBe(1)
    await call(B, 'story_set_reaction', [story.id, 'warmth', false])
    await call(B, 'story_set_reaction', [story.id, 'relate', false])
    expect((await call(A, 'story_get', [story.id])).received_reader_count).toBe(0)
    await call(B, 'story_set_reaction', [story.id, 'warmth', true])
    await owner("UPDATE community_members SET status='removed' WHERE user_id=$1", [B])
    expect((await call(A, 'story_get', [story.id])).received_reader_count).toBe(0)
  })
