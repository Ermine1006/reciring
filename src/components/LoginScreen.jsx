...
export default function LoginScreen() {
  ...
  // Google sign-up is hidden from new users during MVP testing.
  // Existing Google-authenticated accounts can still log in by visiting
  // /?google=1, which reveals the button. Backend provider stays enabled.
  const [showGoogle, setShowGoogle] = useState(() => {
    try { return new URLSearchParams(window.location.search).has('google') }
    catch { return false }
  })
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="flex flex-col gap-2 mt-5">
            ...
            {showGoogle && (
              <>
                ...
                {/* Google OAuth — only shown for existing Google users via ?google=1 */}
                <button
                  type="button"
                  onClick={async () => {
        ...
        {!showGoogle && (
          <p className="text-center mt-3" style={{ fontSize: 10, color: C.textMuted, lineHeight: 1.5 }}>
