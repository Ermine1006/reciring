...
export default function BuddyChoiceProgram({onBack,registerNavigationGuard}) {
 ...
 function navigate(next){if(dirty.current&&!window.confirm('Leave without sending your changes?'))return;dirty.current=false;setView(next)}
 async function join(){
  if(!joinRole)return
 ...
 <header className="bc-program-header"><Button className="bc-back" onClick={()=>{if(!dirty.current||window.confirm('Leave without sending your changes?'))onBack()}}>‹ Together</Button><h1>Buddy Program</h1><p>{upper?'Your crew. A little help, together.':role==='first'?'A little help goes a long way.':'Ask for support or share what you have learned.'}</p></header>
 ...
 :!role?<section className="bc-card bc-join"><h2>How would you like to take part?</h2><p>Choose your year. Find your crew.</p><fieldset className="bc-role-options" disabled={busy}><legend>Your year</legend><label><input type="radio" name="buddy-year" value="upper" checked={joinRole==='upper'} onChange={()=>setJoinRole('upper')}/><span><strong>I am an upper year student</strong><small>Browse requests and choose where to help. No approval needed.</small></span></label><label><input type="radio" name="buddy-year" value="first" checked={joinRole==='first'} onChange={()=>setJoinRole('first')}/><span><strong>I am a first year student</strong><small>Share what you need and what you enjoy helping with.</small></span></label></fieldset><p className="bc-audience">My Buddies uses real names. School pairings stay separate from community help.</p><Button primary disabled={busy||!joinRole||!data.enabled} onClick={join}>{busy?'Joining…':joinRole==='upper'?'Join as a mentor':joinRole==='first'?'Join as a student':'Join Buddy Program'}</Button>{!data.enabled&&<p>Joining is paused by the program coordinator.</p>}</section>
