...
function Button({ primary, children, ...props }) { return <button className="bc-button" style={primary?matchaCta:undefined} type="button" {...props}>{children}</button> }
function Post({ post }) { return <><div className="bc-meta"><span>{post.helpType?.[0] || 'Advice'}</span><span>{post.time}</span><small>{post.is_anonymous?'First-year student':'Jamie · First-year student'}</small></div><p className="bc-label">Looking for</p><p className="bc-copy">{post.needs}</p><p className="bc-label bc-give">Also happy to help with</p><p className="bc-copy">{post.offers || 'Open to discovering how I can help.'}</p><div className="bc-tags">{post.tags?.map(t=><span key={t}>{t}</span>)}</div></> }
function Preview({post,onClose,children}) {
