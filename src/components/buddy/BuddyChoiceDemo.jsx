...
export default function BuddyChoiceDemo({onBack}) {
 const [recommendations,setRecommendations]=useState(DEMO_BUDDIES)
 ...
 return <AppScreen background="transparent"><main className="bc-shell" ref={root}>
  ...
  {role==='first'&&<RecommendationCards items={recommendations} onRefresh={()=>setRecNotice('Sample recommendations refreshed. Skipped profiles remain hidden.')} notice={recNotice} onAct={(item,status)=>{setRecommendations(v=>status==='skipped'?v.filter(x=>x.id!==item.id):v.map(x=>x.id===item.id?{...x,status}:x));setRecNotice(status==='interested'?'Sample interest sent. The upper-year buddy can choose whether to invite you.':'Sample suggestion hidden.')}}/>}
  </>}
