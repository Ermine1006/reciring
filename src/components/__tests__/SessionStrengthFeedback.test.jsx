...
const api = vi.hoisted(() => ({ feedback: vi.fn(), strengths: vi.fn(), share: vi.fn() }))
vi.mock('../../lib/practice', () => ({ fetchFeedbackSupport: api.feedback, fetchPeerStrengthSupport: api.strengths, peerStrengthSharing: api.share }))
afterEach(() => { cleanup(); vi.resetAllMocks() })
it('offers strengths in the chat completion path, allows deselection, and submits them with confirmation', async () => {
 api.feedback.mockResolvedValue({ supported: true }); api.strengths.mockResolvedValue({ supported: true })
 const submit = vi.fn()
 ...
 expect(submit).toHaveBeenCalledWith(expect.objectContaining({ strengthSkills: ['structuring'], outcome: 'completed', suggestionCode: null }))
})
it('does not pretend to collect strengths before the migration is installed', async () => {
 api.feedback.mockResolvedValue({ supported: false }); api.strengths.mockResolvedValue({ supported: false })
