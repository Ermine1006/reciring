...
it('shows sourced reasons and response evidence without identity or a made up score',()=>{
 ...
 expect(screen.getByText(/They can support your focus: Synthesis/)).toBeTruthy()
 expect(screen.getByText('Chosen by this member')).toBeTruthy()
 expect(screen.getByText('Answered 4 of 5 invitations within 48h')).toBeTruthy()
...
it('does not turn absent history into a low score',()=>{
 ...
 expect(screen.getByText('No shared invitation response history yet')).toBeTruthy()
 expect(screen.getByText('No shared partner feedback yet')).toBeTruthy()
