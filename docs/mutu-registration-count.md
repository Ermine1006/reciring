...
The homepage reads a separate scalar RPC and, per the founder’s copy preference, labels the number “members you can turn to for help”. The number still counts all Mutu registered accounts, not only Rotman members; it is not a count of members currently available or a promise of a response. Map nodes and community exchange statistics keep their existing scope and permissions; their captions explicitly say “in this community”. No Auth rows, identities or status breakdowns are exposed. Only authenticated callers can request the aggregate.
