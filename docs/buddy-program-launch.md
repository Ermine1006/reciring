...
3. Set Vercel `VITE_BUDDY_ENABLED=true` and redeploy to expose the Together entry. For a single-browser check first, `localStorage.setItem('mutu_buddy','on')` and refresh. The public `/buddy-demo` does not require this flag.
